...
class FedHEONN_coordinator:
    ...
    def aggregate_parcial(self, M_list, US_list):
        # Number of classes
        ...
        for c in range(0, nclasses):
            ...
            #if (not self.M_glb and not self.US_glb) or init:
    ...
    def calculate_weights(self):
        self.W = []
        if not self.M_glb or not self.U_glb or not self.U_glb:
            return None
        else:
    ...
    def aggregate(self, M_list, US_list):
        ...
        for c in range(0, nclasses):
            ...
            for M_k, US_k in zip(M_rest, US_rest):
                M = M + M_k    
                concatenated = np.concatenate((US_k, US), axis=1)
                U, S, _ = sp.linalg.svd(concatenated, full_matrices=False)
                US = U @ np.diag(S)
